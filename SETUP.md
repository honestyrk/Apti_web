...
All three seed files are additive and safe to run once each, in order, on a fresh database; don't re-run any of them, since re-running would try to insert duplicate topics.

Together these create the full category/branch/topic tree (Quantitative Aptitude, Logical Reasoning, Verbal Ability, Data Interpretation, and Technical with 5 engineering branches) plus 300+ sample MCQs across every topic, so you can test Practice and Test mode immediately. You can add more questions later through the Admin Panel.
