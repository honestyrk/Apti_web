  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-4 py-12">
      <Card className="p-8">
        ...
        <h1 className="mt-5 font-heading text-2xl font-bold text-slate-900 dark:text-white">Welcome back</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Log in to continue your placement prep.</p>
        ...
        <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
          ...
          {unconfirmed && (
            <div className="rounded-lg bg-amber-50 px-3.5 py-2.5 text-sm text-amber-800 dark:bg-amber-950/40 dark:text-amber-300">
              <p>Your email hasn't been confirmed yet. Check your inbox for the confirmation link.</p>
              <button
                type="button"
                onClick={handleResend}
                disabled={resending}
                className="mt-2 font-medium text-amber-900 underline underline-offset-2 disabled:opacity-60 dark:text-amber-200"
              >
                {resending ? 'Resending…' : 'Resend confirmation email'}
              </button>
              {resendMessage && <p className="mt-1 text-amber-700 dark:text-amber-400">{resendMessage}</p>}
            </div>
          )}
        ...
        <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
          ...
          <Link to="/signup" className="font-medium text-brand-700 hover:underline dark:text-brand-300">Sign up</Link>
        </p>
      </Card>
    </div>
  )
