    ...
    <div className="mx-auto max-w-xl px-4 py-10 sm:px-6">
      ...
      <Card className="mt-6 p-6">
        <div className="mb-6 flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-100 text-lg font-semibold text-brand-800 dark:bg-brand-900/40 dark:text-brand-200">
            {(profile?.full_name || user.email)[0]?.toUpperCase()}
          </div>
          <div>
            <p className="font-medium text-slate-900 dark:text-white">{profile?.full_name || 'Unnamed user'}</p>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              value={user.email}
              disabled
              className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-400"
            />
          </div>
